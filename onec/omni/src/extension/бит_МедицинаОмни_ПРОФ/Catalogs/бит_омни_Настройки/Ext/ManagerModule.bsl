...
Функция ТаблицаОписанияКонстант() Экспорт
	...
	НоваяКонстанта = ДобавитьОписаниеКонстанты(
		ТаблицаОписанияКонстант,
		"MAX_VDS_URL",
		НСтр("ru='Адрес VDS для обмена MAX'"),
		ОбщегоНазначения.ОписаниеТипаСтрока(250),
		Раздел);
	НоваяКонстанта.ОбязательноеЗаполнение = Ложь;
	НоваяКонстанта.Описание = НСтр("ru='Публичный HTTPS-адрес backend/VDS без завершающего слэша, например https://max.example.ru'");

	НоваяКонстанта = ДобавитьОписаниеКонстанты(
		ТаблицаОписанияКонстант,
		"MAX_VDS_API_KEY",
		НСтр("ru='Сервисный ключ обмена с VDS'"),
		ОбщегоНазначения.ОписаниеТипаСтрока(250),
		Раздел);
	НоваяКонстанта.ОбязательноеЗаполнение = Ложь;
	НоваяКонстанта.РежимПароля = Истина;
	НоваяКонстанта.Описание = НСтр("ru='Секретный ключ для заголовка X-Onec-Api-Key при обмене 1С с backend/VDS'");

	НоваяКонстанта = ДобавитьОписаниеКонстанты(
		ТаблицаОписанияКонстант,
		"MAX_VDS_POLL_LIMIT",
		НСтр("ru='Лимит сообщений MAX за один опрос VDS'"),
		Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 0, ДопустимыйЗнак.Неотрицательный)),
		Раздел);
	НоваяКонстанта.ЗначениеПоУмолчанию = 50;
	НоваяКонстанта.ОбязательноеЗаполнение = Ложь;
	НоваяКонстанта.Описание = НСтр("ru='Максимальное количество входящих сообщений, которое регламентное задание забирает из очереди VDS за один запрос'");

	НоваяКонстанта = ДобавитьОписаниеКонстанты(
		ТаблицаОписанияКонстант,
		"MAX_VDS_LONG_POLL_WAIT_MS",
		НСтр("ru='Ожидание long polling VDS, мс'"),
		Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(5, 0, ДопустимыйЗнак.Неотрицательный)),
		Раздел);
	НоваяКонстанта.ЗначениеПоУмолчанию = 25000;
	НоваяКонстанта.ОбязательноеЗаполнение = Ложь;
	НоваяКонстанта.Описание = НСтр("ru='Сколько миллисекунд backend удерживает запрос 1С при пустой очереди входящих MAX-сообщений'");
