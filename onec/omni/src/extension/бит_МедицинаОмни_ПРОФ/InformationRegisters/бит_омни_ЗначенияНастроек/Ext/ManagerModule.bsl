...
Функция ЗначениеКонстанты(Идентификатор, Разрез = Неопределено) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	бит_омни_ЗначенияНастроек.Значение КАК Значение
	|ИЗ
	|	РегистрСведений.бит_омни_ЗначенияНастроек КАК бит_омни_ЗначенияНастроек
	|ГДЕ
	|	бит_омни_ЗначенияНастроек.Константа.Идентификатор = &Идентификатор
	|	И &ОтборПоРазрезу";
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоРазрезу", "бит_омни_ЗначенияНастроек.Разрез = &Разрез");
	...
	Иначе
		Справочники.бит_омни_Настройки.НайтиСоздатьКонстанту(Идентификатор);
	КонецЕсли;
...
Функция ЗначенияКонстант(Знач ИдентификаторыСтрокой) Экспорт
	...
	|		ЕСТЬNULL(бит_омни_ЗначенияНастроек.Значение, НЕОПРЕДЕЛЕНО) КАК Значение
	|	ИЗ
	|		Справочник.бит_омни_Настройки КАК бит_омни_Настройки
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.бит_омни_ЗначенияНастроек КАК бит_омни_ЗначенияНастроек
	|			ПО (бит_омни_ЗначенияНастроек.Константа = бит_омни_Настройки.Ссылка)
	|	ГДЕ
	|		бит_омни_Настройки.Идентификатор В(&СписокИдентификаторов)) КАК ЗначенияКонстант
	|
	|СГРУППИРОВАТЬ ПО
	...
	Для каждого СтрокаТаблицыКонстант Из ТаблицаКонстант Цикл
		...
		ОписаниеКонстанты = бит_омни_ОбщегоНазначенияПолныеПрава.ОписаниеКонстанты(СтрокаТаблицыКонстант.Идентификатор);
	...
	Для каждого ДанныеЗначенияКонстанты Из ЗначенияКонстант Цикл
		...
		Если ТаблицаКонстант.НайтиСтроки(ПараметрыПоиска).Количество() = 0 Тогда
			Справочники.бит_омни_Настройки.НайтиСоздатьКонстанту(ДанныеЗначенияКонстанты.Ключ);
		КонецЕсли;
		
		ОписаниеКонстанты = бит_омни_ОбщегоНазначенияПолныеПрава.ОписаниеКонстанты(ДанныеЗначенияКонстанты.Ключ);
...
Функция ФункциональнаяОпцияИспользуется(Идентификатор) Экспорт	
	...
	|ГДЕ
	|	бит_омни_ЗначенияНастроек.Константа.Идентификатор = &Идентификатор
	|	И бит_омни_ЗначенияНастроек.Значение = ИСТИНА";
...
// Возвращает таблицу значений всех констант по регистру бит_омни_ЗначенияНастроек.
//
// Возвращаемое значение:
//     ТаблицаЗначений:
...
Функция ЗначенияВсехКонстант() Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	бит_омни_ЗначенияНастроек.Константа КАК Константа,
	|	бит_омни_ЗначенияНастроек.Константа.Идентификатор КАК Идентификатор,
	|	бит_омни_ЗначенияНастроек.Разрез КАК Разрез,
	|	бит_омни_ЗначенияНастроек.Значение КАК Значение
	|ИЗ
	|	РегистрСведений.бит_омни_ЗначенияНастроек КАК бит_омни_ЗначенияНастроек
	|ГДЕ
	|	НЕ бит_омни_ЗначенияНастроек.Константа.ПометкаУдаления
	|	И бит_омни_ЗначенияНастроек.Константа.Идентификатор <> """"
	|
	|УПОРЯДОЧИТЬ ПО
	|	Идентификатор,
	|	Разрез";
...
// Формирует запись в регистр бит_омни_ЗначенияНастроек по указанной константе и значению. 
//
// Параметры:
//  Константа - СправочникСсылка.бит_омни_Настройки - Ссылка на константу.
...
Процедура УстановитьЗначениеКонстантыБезРазреза(Константа, Значение) Экспорт
	...
	МенеджерЗаписи = РегистрыСведений.бит_омни_ЗначенияНастроек.СоздатьМенеджерЗаписи();
...
// Формирует записи в регистр бит_омни_ЗначенияНастроек по указанной константе и значениям с разрезами. 
//
// Параметры:
//  Константа - СправочникСсылка.бит_омни_Настройки - Ссылка на константу.
...
Процедура УстановитьЗначенияКонстантыСРазрезом(Константа, Значения) Экспорт
	...
	НаборЗаписей = РегистрыСведений.бит_омни_ЗначенияНастроек.СоздатьНаборЗаписей();
...
Процедура УстановитьЗначениеКонстантыПоРазрезу(Идентификатор, Разрез, Значение) Экспорт

	Константа = Справочники.бит_омни_Настройки.НайтиСоздатьКонстанту(Идентификатор);
	
	МенеджерЗаписи = РегистрыСведений.бит_омни_ЗначенияНастроек.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.Константа = Константа;
	МенеджерЗаписи.Разрез = Разрез;
	МенеджерЗаписи.Значение = Значение;
